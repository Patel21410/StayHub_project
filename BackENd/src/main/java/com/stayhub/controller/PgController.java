...
public class PgController {
    ...
    @GetMapping("/allPgList")
    public ResponseEntity<List<Pg>> allPgList(){
    	List<Pg> allPgs = pgService.allPg();
    	return ResponseEntity.status(200).body(allPgs);
    	
    }
    @GetMapping("/samePgList")
    public ResponseEntity<List<Pg>> samePgList(){
    	List<Pg> allPgs = pgService.fivePg();
    	return ResponseEntity.status(200).body(allPgs);
